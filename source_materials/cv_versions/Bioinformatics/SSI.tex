...
\textbf{Bioinformatics Software \& Pipelines Development} \\
During the past two years, I've been working on the development, maintenance and update of \href{https://github.com/kvittingseerup/IsoformSwitchAnalyzeR}{IsoformSwitchAnalyzeR}. I regularly checked and solved the issues on Github, then planned and implemented the necessary update for ISAR. Now we've updated it to version 2.0 and applied it to both long-read (Alzheimer's) and single-cell (Glioblastoma) RNAseq data to decode the disease mechanisms on the isoform level (preprint). 

        \vspace{0.3 cm}

\textbf{Development of R Shiny Dashboards} \\
Now I'm doing my master's thesis, which focuses on the development of quantitative methods to assess batch-to-batch variability in single-cell RNAseq data. As there's no single perfect method to fulfil our needs, we're designing and implementing interactive R Shiny dashboards to integrate and visualise results from multiple computational methods.
